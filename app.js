...
/*
// function to convert text to sentence case for an element by ID
function toSentenceCase(loggedUser) {
    const element = document.getElementById(elementId);
    if (element && element.textContent) {
        element.textContent =
            element.textContent.trim().charAt(0).toUpperCase() +
            element.textContent.trim().slice(1).toLowerCase();
    }
};
*/
